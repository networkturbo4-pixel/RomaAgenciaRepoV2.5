...
try {
    ...
    if ($pay_id > 0) {
        ...
        $stmt = $db->prepare("UPDATE employee_payments SET payment_date=?, concept=?, amount=?, extra_payment=?, extra_days=?, extra_hours=?, voucher_url=?, status=? WHERE id=?");
        $stmt->execute([$payment_date, $concept, $amount, $extra_amount, $extra_days, $extra_hours, $final_voucher, $status, $pay_id]);
        ...
        // Insert new payment
        $stmt = $db->prepare("INSERT INTO employee_payments (employee_id, payment_date, concept, amount, extra_payment, extra_days, extra_hours, voucher_url, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$employee_id, $payment_date, $concept, $amount, $extra_amount, $extra_days, $extra_hours, $voucher_url, $status]);
