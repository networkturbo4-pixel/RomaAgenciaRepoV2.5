const RrhhModule = (function() {
    ...
    function setupEventListeners() {
        ...
        // OCR for payment voucher
        const payVoucher = document.getElementById('pay_voucher');
        if (payVoucher) {
            payVoucher.addEventListener('change', async (e) => {
                const file = e.target.files[0];
                if (!file || !file.type.startsWith('image/')) return;
                
                const btn = document.querySelector('#payment_form button[type="submit"]');
                const originalHtml = btn.innerHTML;
                btn.innerHTML = '<i class="ph ph-spinner ph-spin" style="animation: spin 1s linear infinite;"></i> Analizando imagen...';
                btn.disabled = true;
                ...
                try {
                    ...
                    // Extraer total
                    const amountMatch = text.match(/(?:S\/|TOTAL|IMPORTE|Soles)[\s:S\/.-]*([\d.,]+)/i) || text.match(/\b\d{2,5}\.\d{2}\b/);
                    if (amountMatch) {
                    ...
                    document.getElementById('pay_date').value = `${yyyy}-${mm}-${dd}`;
                    
                    // Concepto (Nómina + mes)
                    const months = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];
                    const monthName = months[dateObj.getMonth()];
                    document.getElementById('pay_concept').value = `Nómina ${monthName}`;
    ...
    function updateCalculator() {
        ...
        const extraAmount = (extraDays * dailyRate) + (extraHours * hourlyRate);
